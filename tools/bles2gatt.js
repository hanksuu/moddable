...
class AMBDGATTFile extends GATTFile {
	generate() {
		let tool = this.tool;
		let file = this.file;
		let services = this.services;
		...
		if (0 == services.length) {
			...
			file.line("#define max_attribute_count 0");
			file.line("");
			file.line("static const struct T_ATTRIB_APPL modd_ble_service_tbl[] = {};");
			file.line("static const uint8_t attribute_counts[0] = {};");
			file.line("static const char_name_table char_names[0] = {};");
			return;
		}
		...
		services.forEach((service, index) => {
			...
			if (4 == service.uuid.length) {
				file.write(`#define GATT_SERVICE${index}_UUID\t\t`);
				file.write(`0x${service.uuid}`);
			}
			...
			for (let key in characteristics) {
				...
				if (4 == characteristic.uuid.length) {
					file.write(`#define GATT_SERVICE${index}_CHR${characteristicIndex}_UUID\t\t`);
					file.write(`0x${characteristic.uuid}`);
				}
				file.line("");
				++characteristicIndex;
		...
		file.line(`#define max_attribute_count ${maxAttributeCount}`);
		file.line("");
		file.line(`const T_ATTRIB_APPL modd_ble_service_tbl[] =`);
		file.line("{");
		services.forEach((service, index) => {
			characteristicIndex = 0;
			file.line(`\t // Primary Service ${service.uuid}`);
			file.line("\t{");
			file.line("\t\t(ATTRIB_FLAG_VALUE_INCL | ATTRIB_FLAG_LE),");
			file.line("\t\t{");
			file.line("\t\t\tLO_WORD(GATT_UUID_PRIMARY_SERVICE),");
			file.line("\t\t\tHI_WORD(GATT_UUID_PRIMARY_SERVICE),");
			file.line(`\t\t\tLO_WORD(GATT_SERVICE${index}_UUID),`);
			file.line(`\t\t\tHI_WORD(GATT_SERVICE${index}_UUID)`);
			file.line("\t\t},");
			file.line("\t\tUUID_16BIT_SIZE,");
			file.line("\t\tNULL,");
			file.line("\t\tGATT_PERM_READ");
			file.line("\t},");
			++attributeIndex;
			...
			for (let key in characteristics) {
				...
				let properties = this.parseProperties(characteristic.properties.split(","));
				let permissions = this.parsePermissions(characteristic.permissions.split(","));
				file.line(`\t // Characteristic${characteristicIndex}`);
				file.line("\t{");
				file.line("\t\tATTRIB_FLAG_VALUE_INCL,");
				file.line("\t\t{");
				file.line("\t\t\tLO_WORD(GATT_UUID_CHARACTERISTIC),");
				file.line("\t\t\tHI_WORD(GATT_UUID_CHARACTERISTIC),");
				...
				file.line("\t{");
				file.line("\t\tATTRIB_FLAG_VALUE_APPL,");
				file.line("\t\t{");
				file.line(`\t\t\tLO_WORD(GATT_SERVICE${index}_CHR${characteristicIndex}_UUID),`);
				file.line(`\t\t\tHI_WORD(GATT_SERVICE${index}_CHR${characteristicIndex}_UUID)`);
				file.line("\t\t},");
				file.line("\t\t0,");
				file.line("\t\tNULL,");
				file.line(`\t\t${permissions}`);
				...
				if (characteristic._notify) {
					file.line("\t{");
					file.line("\t\tATTRIB_FLAG_VALUE_INCL | ATTRIB_FLAG_CCCD_APPL,");
					file.line("\t\t{");
					file.line("\t\t\tLO_WORD(GATT_UUID_CHAR_CLIENT_CONFIG),");
					file.line("\t\t\tHI_WORD(GATT_UUID_CHAR_CLIENT_CONFIG),");
					file.line("\t\t\tLO_WORD(GATT_CLIENT_CHAR_CONFIG_DEFAULT),");
					file.line("\t\t\tHI_WORD(GATT_CLIENT_CHAR_CONFIG_DEFAULT)");
					file.line("\t\t},");
					file.line("\t\t2,");
					file.line("\t\tNULL,");
					file.line("\t\t(GATT_PERM_READ | GATT_PERM_WRITE)");
					file.line("\t},");
					++attributeIndex;
				}
				++characteristicIndex;
	...
	parseProperties(properties) {
		...
		properties.forEach(p => {
			switch(p.trim()) {
				case "read":
					props.push("GATT_CHAR_PROP_READ");
					break;
				case "write":
					props.push("GATT_CHAR_PROP_WRITE");
					break;
				case "writeNoResponse":
					props.push("GATT_CHAR_PROP_WRITE_NO_RSP");
					break;
				case "notify":
					props.push("GATT_CHAR_PROP_NOTIFY");
					break;
				case "indicate":
					props.push("GATT_CHAR_PROP_INDICATE");
					break;
				case "extended":
					props.push("GATT_CHAR_PROP_EXT_PROP");
					break;
				default:
					throw new Error("unknown property");
			}
...
export default class extends TOOL {
	...
	run() {
		...
		if ("esp32" == this.platform) {
			...
			gatt = new AMBDGATTFile(dictionary);
		gatt.generate();
		file.close();
	}
}
